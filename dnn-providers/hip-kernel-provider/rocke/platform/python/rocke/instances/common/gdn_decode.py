...
assumed contiguous (row-major), matching the AITER packed/FlyDSL decode contract:
...
VGPR-heavy by design; the warp-tiled reduction (mirroring FlyDSL's 8x8 layout)
is the first optimization pass, not part of the correctness baseline.
"""
...
from ...helpers.reduction import tree_reduce
...
class GdnDecodeSpec:
    """One GDN single-token decode instance (Qwen3-Next dims by default)."""
    ...
    wave_size: int = 64
    # Tuned across the full decode batch range (B=1..256) on gfx950 vs AITER FlyDSL:
    # (num_warps=2, warp_threads_k=16, blocks_per_v_dim=8) matches-or-beats FlyDSL's
    # device time at every batch (HIP-graph-measured, ROCm 7.14). See the vault
    # "GDN decode tuned configs" note for the per-batch table (a future dispatcher).
    num_warps: int = 2
...
def build_gdn_decode(spec: GdnDecodeSpec) -> KernelDef:
    """Build the IR for one GDN single-token decode instance (dispatch)."""
    ok, why = is_valid_spec(spec)
    if not ok:
        raise ValueError(f"invalid gdn_decode spec: {why}")
...
def _build_warp_tiled(spec: GdnDecodeSpec) -> KernelDef:
    """v2: FlyDSL-style warp tiling. One CTA per (seq, value-head); the
