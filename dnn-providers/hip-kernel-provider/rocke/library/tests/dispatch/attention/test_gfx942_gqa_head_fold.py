...
def _problem(dtype="bf16", block_size=16, head_size=128, sliding_window=4096,
             num_query_heads=32, num_kv_heads=8, sq=8192):
...
class TestGfx942GqaHeadFoldLaunchGrid(unittest.TestCase):
    ...
    def test_fold_grids_over_kv_heads(self):
        ...
        self.assertEqual(gy, p.total_q // 32 + p.num_seqs, "fold grid.y = 32-token blocks")
    ...
    def test_nonfold_grids_over_query_heads(self):
        ...
        gx, _, _ = self._grid(p)
        self.assertEqual(gx, p.num_query_heads, "non-fold grid.x must be num_query_heads")


class TestGfx942GqaHeadFoldEmit(unittest.TestCase):
    def _ir(self, dtype, block_size=16):
        with _PinArch("gfx942"):
            spec = _tiled_spec_from_problem(_problem(dtype=dtype, block_size=block_size))
    ...
    def test_fold_emits_distinct_kernel_name(self):
        ...
        self.assertTrue(re.search(r"_4wgqa_fold\b", ir), "fold kernel must emit `_4wgqa_fold`")

    def test_nonfold_keeps_baseline_name(self):
        ir = self._ir("fp16")
        self.assertFalse(re.search(r"_4wgqa_fold\b", ir), "fp16 must not fold")
        self.assertTrue(re.search(r"_4wgqa\b", ir), "fp16 keeps the baseline `_4wgqa` name")
